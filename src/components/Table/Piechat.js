...
class Piechart extends Component {
    ...
    render() {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            if (i == (items.length) - 1) {
                color = color + "var(--" + items[i].color + ") " + (100 - newItemPercentage).toString() + "%)";
            } else {
                color = color + "var(--" + items[i].color + ") " + previousItemPercentage.toString() + " " + (100 - newItemPercentage).toString() + "%,";
            }
                ...
                <div id="pie-chart-container">
                    ...
                    <div id="legenda">
                        {this.props.dataList.map((item) => (
                            <div class="entry">
                                <div id={item.color} class="entry-color"></div>
